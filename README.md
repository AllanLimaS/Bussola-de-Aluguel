...
## A Motivação (Por que criei este projeto?)

Este projeto nasceu de uma necessidade pessoal real. Durante a minha busca por um apartamento para alugar, me deparei com um problema frustrante: a necessidade de acessar vários portais imobiliários diariamente, apenas para rolar por dezenas de anúncios repetidos ou imóveis que eu já havia rejeitado mentalmente nos dias anteriores. 
...
- **Recomendação Personalizada:** A listagem padrão é sempre ordenada pelo score de similaridade.
...
- **Recomendação Personalizada:** Algoritmo ponderado que considera:
...
A maneira mais fácil e robusta de rodar o projeto, incluindo a rotina de automação de web scraping, é utilizando o Docker.

1. Instale o [Docker Desktop](https://www.docker.com/products/docker-desktop).
2. Na pasta raiz do projeto, certifique-se de que configurou o arquivo `backend/.env` (use o `.env.example` como base).
...
O Docker fará o build das imagens (API, Scraper e Frontend). O site ficará disponível em `http://localhost:5173`.
*Dica:* O robô de extração (scraper) rodará uma vez automaticamente no momento da inicialização e depois de forma contínua a cada 12 horas, garantindo que o seu banco de dados esteja sempre alimentado.

---

## Como Executar o Projeto Localmente (Sem Docker)

Para rodar este projeto na sua máquina de forma manual, siga os passos abaixo:
